...
//		fprintf(out,"%s\n%s\n+\n%s\n",(arrays+i)->name,(arrays+i)->seq,(arrays+i)->quality);
		sds packed_sds = seq2sds((arrays+i)->seq);
		seqlen=(uint8_t)strlen((arrays+i)->seq);
		if (!i){
			fwrite(&seqlen,sizeof(uint8_t),1,out);
			seqlen=(uint8_t)sdslen(packed_sds);
			fwrite(&seqlen,sizeof(uint8_t),1,out);
		}
		fwrite(packed_sds,sdslen(packed_sds),1,out);
	}
	free(arrays);
